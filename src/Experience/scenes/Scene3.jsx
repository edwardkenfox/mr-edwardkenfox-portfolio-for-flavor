...
export default function Scene3({ x0, data }) {
  ...
  return (
    <group>
      ...
      {data.follow && <Handwriting lines={data.follow} size={0.13} w={3.4} h={0.6} align="center" position={[x0 + 9.2, 0.8, -0.55]} rotation={[0, 0, -0.02]} url={data.followUrl} />}
      {data.photo && <PaperPhoto src={data.photo.src} label={data.photo.label} caption={data.photo.caption} w={1.9} h={2.1} position={[x0 + 12.3, 2.1, -0.4]} tilt={0.05} url={c2.url} />}
      ...
      <PaperSprite paint={S.bagel} w={0.7} h={0.7} position={[x0 + 15.0, 3.6, -1.4]} rotation={[0, 0, -0.4]} sway={0.03} seedOffset={9} />
